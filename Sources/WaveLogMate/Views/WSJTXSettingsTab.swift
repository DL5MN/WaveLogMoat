  ...
  public var body: some View {
    Form {
      Section("Receive from WSJT-X") {
        ...
        Text(
          "WSJT-X sends UDP data to this address. Use 127.0.0.1 for local connections or 0.0.0.0 to accept from other machines on the network."
        )
        .font(.callout)
      ...
      Section("Protocol") {
        ...
        case .text:
          ...
          Text(
            "Receives logged QSOs as ADIF text from the WSJT-X Secondary UDP Server. Simple and reliable — works alongside JT-Bridge, GridTracker, and other tools without conflict."
          )
          .font(.callout)
          .foregroundStyle(.secondary)
        ...
        case .binary:
          ...
          Text(
            "Receives logged QSOs and real-time status updates (frequency, mode, DX call) from the WSJT-X primary UDP port. Only one application can use this port — do not use if JT-Bridge or GridTracker need it."
          )
          .font(.callout)
          .foregroundStyle(.secondary)

          Label(
            "Only one application can receive on the primary UDP port. This will conflict with JT-Bridge, GridTracker, or any other tool using this port.",
