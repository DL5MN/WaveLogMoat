...
public struct WavelogSettingsTab: View {
  ...
  public var body: some View {
    Form {
      Section("Server") {
        TextField("URL", text: $appState.config.wavelogURL, prompt: Text("log.example.com"))
          .textContentType(.URL)
      ...
      Section("Station Profile") {
        ...
        HStack {
          Button {
            Task {
              ...
              if appState.wavelogConnectionStatus == .error {
                stationProfilesErrorMessage =
                  appState.lastError ?? "Failed to fetch station profiles"
              }
            }
          } label: {
            HStack(spacing: 4) {
          ...
          .disabled(
            appState.apiKey.isEmpty || appState.config.wavelogURL.isEmpty || isFetchingProfiles)
      ...
      Section("Connection Test") {
        HStack {
          ...
          .disabled(
            appState.apiKey.isEmpty || appState.config.wavelogURL.isEmpty
              || appState.config.stationProfileID.isEmpty || isTestingConnection)
    ...
    .formStyle(.grouped)
    .onAppear {
      if !appState.apiKey.isEmpty && !appState.config.wavelogURL.isEmpty
        && appState.stationProfiles.isEmpty
      {
        Task {
          stationProfilesErrorMessage = nil
          await appState.fetchStationProfiles()
