...
public final class QDataStreamReader: @unchecked Sendable {
  public enum ReaderError: Error, LocalizedError {
    ...
    public var errorDescription: String? {
      ...
      case .insufficientData(let operation, let expected, let remaining):
        return
          "Insufficient data while reading \(operation): expected \(expected) bytes, remaining \(remaining)"
  ...
  public func parseMessage(_ data: Data) throws -> WSJTXParsedMessage {
    ...
    case .heartbeat:
      ...
      return .heartbeat(
        clientId: clientId, maxSchema: maxSchema, version: version, revision: revision)
  ...
  private func readRaw(byteCount: Int, operation: String) throws -> Data {
    ...
    guard bytesRemaining >= byteCount else {
      throw ReaderError.insufficientData(
        operation: operation, expected: byteCount, remaining: bytesRemaining)
    }
  ...
  private func gregorianDateComponents(fromJulianDay julianDay: Int64) throws -> (
