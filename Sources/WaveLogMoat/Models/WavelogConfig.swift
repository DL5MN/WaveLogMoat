...
public struct WavelogConfig: Codable, Sendable, Equatable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        listenAddress = try container.decodeIfPresent(String.self, forKey: .listenAddress) ?? "127.0.0.1"
        ...
        allowSelfSignedCerts = try container.decodeIfPresent(Bool.self, forKey: .allowSelfSignedCerts) ?? true
        httpTimeout = try container.decodeIfPresent(Int.self, forKey: .httpTimeout) ?? 5000
        showFrequencyInMenuBar = try container.decodeIfPresent(Bool.self, forKey: .showFrequencyInMenuBar) ?? false

        if let protocol_ = try container.decodeIfPresent(UDPProtocol.self, forKey: .udpProtocol) {
            udpProtocol = protocol_
        } else {
            let enableBinaryUDP = try container.decodeIfPresent(Bool.self, forKey: .enableBinaryUDP) ?? false
